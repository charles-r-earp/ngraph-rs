use crate::{ElementType, Shape, Function};
use super::{Tensor, Executable};
...
impl Backend {
  ...
  pub fn create<S>(name: S) -> Result<Self, BackendError>
    ...
    if !backend.is_null() {
      ...
      Err(BackendError{details: format!("Failed to create backend {:?}, it may not exist!", name)})
    }
  }
  #[inline]
  ...
  pub fn compile<'a>(&self, func: &'a Function) -> Result<Executable, CompileError> {
    ...
    if !exec.is_null() {
      ...
      Err(CompileError{details: format!("Failed to compile function!")})
    }
  }
}
